...
async function setupTestEnvironment() {
    ...
    await FileSystem.writeFile(path.join(sourceDir, 'star-icon.svg'), testSvg, 'utf-8');
    await FileSystem.writeFile(path.join(sourceDir, 'circle-icon.svg'), testSvg, 'utf-8');
    await FileSystem.writeFile(path.join(sourceDir, 'menu-icon.svg'), testSvg, 'utf-8');
    console.log('✅ Test environment ready');
}
async function testWebpackPlugin() {
    console.log('\n📦 Testing Webpack Plugin...');
    try {
        const { SvgerWebpackPlugin, svgerLoader } = await import('../../dist/integrations/webpack.js');
        ...
        console.log('  ✅ Plugin has apply method:', typeof plugin.apply === 'function');
        // Test 2: Loader function exists
        console.log('  ✅ Loader function exists:', typeof svgerLoader === 'function');
...
async function testVitePlugin() {
    ...
    try {
        ...
        console.log('  ✅ Plugin has buildStart:', typeof plugin.buildStart === 'function');
        console.log('  ✅ Plugin has handleHotUpdate:', typeof plugin.handleHotUpdate === 'function');
...
async function testRollupPlugin() {
    ...
    try {
        const { svgerRollupPlugin } = await import('../../dist/integrations/rollup.js');
        // Test 1: Plugin creation
...
async function testNextJsPlugin() {
    ...
    try {
        const { withSvger, SvgerNextJsPlugin, configureSvgImports } = await import('../../dist/integrations/nextjs.js');
        ...
        console.log('  ✅ Has webpack function:', typeof nextConfig.webpack === 'function');
        ...
        console.log('  ✅ configureSvgImports exists:', typeof configureSvgImports === 'function');
...
async function testJestPreset() {
    console.log('\n🧪 Testing Jest Preset...');
    try {
        const { svgerJestTransformer, jestPreset, createJestTransformer, } = await import('../../dist/integrations/jest-preset.js');
        // Test 1: Transformer
        console.log('  ✅ Transformer exists:', typeof svgerJestTransformer === 'object');
        console.log('  ✅ Transformer has process:', typeof svgerJestTransformer.process === 'function');
        // Test 2: Preset config
        console.log('  ✅ Preset config exists:', typeof jestPreset === 'object');
        console.log('  ✅ Has transform config:', jestPreset.transform !== undefined);
...
async function testIntegrationExports() {
    ...
    try {
        ...
        console.log('  ✅ SvgerWebpackPlugin exported:', 'SvgerWebpackPlugin' in mainExports);
        console.log('  ✅ svgerVitePlugin exported:', 'svgerVitePlugin' in mainExports);
        console.log('  ✅ svgerRollupPlugin exported:', 'svgerRollupPlugin' in mainExports);
        console.log('  ✅ withSvger exported:', 'withSvger' in mainExports);
        console.log('  ✅ svgerJestTransformer exported:', 'svgerJestTransformer' in mainExports);
...
async function runAllTests() {
    ...
    Object.entries(results).forEach(([name, success]) => {
        const icon = success ? '✅' : '❌';
        console.log(`  ${icon} ${name.padEnd(20)} ${success ? 'PASSED' : 'FAILED'}`);
    });
    console.log('\n' + '='.repeat(60));
