...
async function setup() {
    ...
    await FileSystem.writeFile(path.join(testSvgDir, 'check-circle.svg'), testSVG, 'utf-8');
    await FileSystem.writeFile(path.join(testSvgDir, 'home_icon.svg'), testSVG, 'utf-8');
    await FileSystem.writeFile(path.join(testSvgDir, 'UserProfile.svg'), testSVG, 'utf-8');
    await FileSystem.writeFile(path.join(testSvgDir, 'settings-gear.svg'), testSVG, 'utf-8');
}
/**
...
async function testNamingKebab() {
    ...
    await svgProcessor.processSVGFile(path.join(TEST_OUTPUT_DIR, 'test-svgs', 'check-circle.svg'), outputDir, { framework: 'react', typescript: true });
    // Check if file was created with kebab-case name
    const files = await FileSystem.readDir(outputDir);
    const hasCorrectNaming = files.some(f => f.includes('check-circle') || f.includes('CheckCircle'));
    if (!hasCorrectNaming) {
...
async function testNamingPascal() {
    ...
    await FileSystem.ensureDir(outputDir);
    // Process SVG
    await svgProcessor.processSVGFile(path.join(TEST_OUTPUT_DIR, 'test-svgs', 'home_icon.svg'), outputDir, { framework: 'react', typescript: true });
...
async function testNamingCamel() {
    ...
    await svgProcessor.processSVGFile(path.join(TEST_OUTPUT_DIR, 'test-svgs', 'settings-gear.svg'), outputDir, { framework: 'react', typescript: true });
    const files = await FileSystem.readDir(outputDir);
    if (files.length === 0) {
...
async function testExtensionOverride() {
    ...
    await svgProcessor.processSVGFile(path.join(TEST_OUTPUT_DIR, 'test-svgs', 'check-circle.svg'), outputDir, { framework: 'react', typescript: false });
...
async function testDefaultComponentNaming() {
    ...
    for (const testCase of testCases) {
        ...
        if (componentName !== testCase.expected) {
            throw new Error(`Component name mismatch: expected "${testCase.expected}", got "${componentName}"`);
        }
    }
}
/**
...
async function testFrameworkExtensions() {
    ...
    for (const fw of frameworks) {
        const outputDir = path.join(TEST_OUTPUT_DIR, `framework-${fw.name}-${fw.typescript ? 'ts' : 'js'}`);
        await FileSystem.ensureDir(outputDir);
        await svgProcessor.processSVGFile(path.join(TEST_OUTPUT_DIR, 'test-svgs', 'check-circle.svg'), outputDir, { framework: fw.name, typescript: fw.typescript });
        const files = await FileSystem.readDir(outputDir);
        const hasCorrectExtension = files.some(f => f.endsWith(fw.expected));
        if (!hasCorrectExtension) {
            throw new Error(`Framework ${fw.name} with typescript=${fw.typescript} should generate ${fw.expected} files`);
        }
    }
}
/**
...
async function testBatchProcessing() {
    ...
    if (successCount !== svgFiles.length) {
        throw new Error(`Batch processing failed: ${successCount}/${svgFiles.length} succeeded`);
    }
    // Verify all files were created
    const outputFiles = await FileSystem.readDir(outputDir);
    if (outputFiles.length !== svgFiles.length) {
        throw new Error(`Expected ${svgFiles.length} output files, got ${outputFiles.length}`);
    }
}
/**
...
async function testIndexFileGeneration() {
    ...
    const expectedComponents = ['CheckCircle', 'HomeIcon', 'UserProfile', 'SettingsGear'];
